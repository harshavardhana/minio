...
func getRemoteInstanceClient(host string, port int) (*miniogo.Core, error) {
	...
	client, err := miniogo.NewCore(net.JoinHostPort(host, strconv.Itoa(port)), globalServerConfig.Credential.AccessKey, globalServerConfig.Credential.SecretKey, globalIsSSL)
	if err != nil {
		return nil, err
	}
	return client, nil
