...
// GenerateKey returns a new plaintext key, generated by the in-memory master
// key KSM, and a sealed version of this plaintext key encrypted using the
...
// Therefore it sends the sealedKey to the in-memory master key
// which decrypts it using the named key referenced by keyID
// and responses with the plaintext key.
//
...
//
// The context must be same context as the one provided while generating the
// plaintext key / sealedKey.
